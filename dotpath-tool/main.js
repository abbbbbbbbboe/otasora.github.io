...
//ドットをグリット上に並べる。for文で範囲を作って繰り返し行うことで羅列する。
    for (let y = 0; y <= gridHeight; y += gridSize) {
        for (let x = 0; x <= gridWidth; x += gridSize) {
            ...
            circle.setAttribute("fill", "#ebf6f7"); //svgの塗りの色
...
UIokButtonElement.addEventListener("click", () => {
    const panelHeightValue = Number(panelHeight.value);
    const panelWidthValue = Number(panelWidth.value);
    ...
    gridLayer.innerHTML = "";
    createGrid(panelWidthValue, panelHeightValue);
...
function startDrawing() {
    ...
    currentPolyline.setAttribute("stroke", "black");
...
function downloadSVG() {
    ...
    exportSVG.setAttribute("viewBox", "0 0 500 500");
